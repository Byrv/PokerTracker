...
try {
  ...
  console.log('SESSION');
  console.log({
    id: s.id, name: s.name, status: s.status, created_by: s.created_by,
    chips_per_paise: Number(s.chips_per_paise), played_on: s.played_on,
  });

  const ps = (await c.query('select user_id, joined_at from public.session_participants where session_id = $1', [sessionId])).rows;
  console.log('\nPARTICIPANTS', ps.length);
  for (const p of ps) console.log(' ', p.user_id);

  const bs = (await c.query('select user_id, amount_paise, chips, recorded_at from public.buyins where session_id = $1 order by recorded_at', [sessionId])).rows;
  console.log('\nBUYINS', bs.length);
  let totalBuyin = 0n;
  for (const b of bs) {
  ...
  const cs = (await c.query('select user_id, chip_count, amount_paise, status, submitted_at, confirmed_at from public.cashouts where session_id = $1 order by submitted_at', [sessionId])).rows;
  console.log('\nCASHOUTS', cs.length);
  let totalCashConfirmed = 0n;
  for (const co of cs) {
    console.log(`  user=${co.user_id.slice(0,8)} chip_count=${co.chip_count} amount=${co.amount_paise} status=${co.status}`);
