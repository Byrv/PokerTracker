...
try {
  ...
  const before = (await c.query('select id, name, status, created_by from public.sessions where id = $1', [sessionId])).rows[0];
  if (!before) { console.error('not found'); process.exit(2); }
  ...
  const after = (await c.query('select id from public.sessions where id = $1', [sessionId])).rows[0];
