...
try {
  ...
  const all = await c.query(`select id, name, status, closed_at, opened_at from public.sessions order by opened_at desc`);
  console.log(`\nAll sessions:`);
  for (const s of all.rows) {
    console.log(`  ${s.id.slice(0,8)} ${(s.name ?? '(unnamed)').padEnd(24)} ${s.status.padEnd(8)} closed=${s.closed_at?.toISOString?.() ?? '-'}`);
  }
} finally {
  await c.end();
}
